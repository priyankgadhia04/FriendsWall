...
use FriendsWall\Likes\Like;
use FriendsWall\Likes\InvalidLikeException;
...
try {
    (new Like(intval($_POST['pid']), $_SESSION['id']))->toggleLike();
    $post = (new Post())->setId(intval($_POST['pid']));
    $data['totalLikes'] = $post->getTotalLikes();
    $data['isLiked'] = $post->isLikedBy($_SESSION['id']);
} catch (InvalidLikeException $exc) {
    $success = false;
