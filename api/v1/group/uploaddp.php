...
try {
    ...
    if (!$group->hasAdmin($_SESSION['id'])) {
        $success = false;
        $error = 'Only group admins can change group picture';
        goto output;
    }
    if ($group->isDeleted()) {
        $success = false;
...
if(isset($_FILES['file']))
{
	...
	if(is_file($dst)) {
		@unlink($img);
		$_SESSION['groupdp'][$group->getId()] = $dst;
    ...
    if (!isset($_SESSION['groupdp'][$group->getId()]) || !is_file($_SESSION['groupdp'][$group->getId()])) {
        $success = false;
        $error = 'uploaded file not found';
        goto output;
    }
    $file = $_SESSION['groupdp'][$group->getId()];
    ...
    @unlink($file);
    $dp = substr(str_shuffle("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"),0,9);
    imagejpeg($tmp, $resimg . $dp . '.jpg');
    if (is_file($resimg . $dp . '.jpg')) {
        @unlink($resimg . $group->getDP() . '.jpg');
        try {
            $group->setDP($dp);
            $group->update();
            $data['dp'] = $dp;
        ...
        $success = false;
        $error = 'Unable to change group picture due to internal error';
    }
